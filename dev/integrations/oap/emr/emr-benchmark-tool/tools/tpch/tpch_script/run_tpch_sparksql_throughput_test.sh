...
if [ $1 -ne 2 ]; then
  bash ${tpch_script_home}/run_stream.sh 1 ${dbname_file} | tee -a $LOG 2>&1 &
  bash ${tpch_script_home}/run_stream.sh 2 ${dbname_file} | tee -a $LOG 2>&1 &
  bash ${tpch_script_home}/run_stream.sh 3 ${dbname_file} | tee -a $LOG 2>&1 &
  bash ${tpch_script_home}/run_stream.sh 4 ${dbname_file} | tee -a $LOG 2>&1 &
else
  bash ${tpch_script_home}/run_stream.sh 5 ${dbname_file} | tee -a $LOG 2>&1 &
  bash ${tpch_script_home}/run_stream.sh 6 ${dbname_file} | tee -a $LOG 2>&1 &
  bash ${tpch_script_home}/run_stream.sh 7 ${dbname_file} | tee -a $LOG 2>&1 &
  bash ${tpch_script_home}/run_stream.sh 8 ${dbname_file} | tee -a $LOG 2>&1 &
fi
