...
LOG_HOME=/opt/software/log
sudo chown $(whoami):$(whoami) ${LOG_HOME}
TPCDS_LOG_HOME=${LOG_HOME}/tpcds
TPCH_LOG_HOME=${LOG_HOME}/tpch

echo original parameters=[$@]
args=`getopt -a -o rvcpi:w:d:q:s: -l rerun,validation,cron,plugin,iteration:,workload:,dir:,queue:,send: -- "$@"`
echo ARGS=[$args]
eval set -- "${args}"
...
function create_tpcds_database_file() {
    sudo mkdir -p ${TPCDS_LOG_HOME}
    echo "use tpcds_${format}_scale_${scaleFactor};" > ${TPCDS_LOG_HOME}/dbname.txt
}

function create_tpch_database_file() {
    sudo mkdir -p ${TPCH_LOG_HOME}
    echo "use tpch_${format}_scale_${scaleFactor};" > ${TPCH_LOG_HOME}/dbname.txt
}
...
function create_tpch_datagen_script() {
    ...
    echo "import com.databricks.spark.sql.perf.tpch._
        ...
        tables.createExternalTables(data_path, format, databaseName, overwrite = true, discoverPartitions = false)" > ${TPCH_LOG_HOME}/tpch_datagen.scala
}
...
if [ "${runType}" = "rerun" ]; then
            ...
            for t in $(seq 99);do
                if [ $t == 14 ] || [ $t == 23 ] || [ $t == 24 ] || [ $t == 39 ]; then
                    if [ -e "${queriesdir}/q${t}a.sql" ]; then
                        start=$(date +%s)
                        $SPARK_PHIVE_HOME/bin/beeline -u jdbc:hive2://$(hostname):10000 -i ${TPCDS_LOG_HOME}/dbname.txt -f ${queriesdir}/q${t}a.sql > ${TPCDS_LOG_HOME}/${round}/q${t}a.log 2>&1
                        end=$(date +%s)
                        time=$(( $end - $start ))
                    ...
                    if [ -e "${queriesdir}/q${t}b.sql" ]; then
                        start=$(date +%s)
                        $SPARK_PHIVE_HOME/bin/beeline -u jdbc:hive2://$(hostname):10000 -i ${TPCDS_LOG_HOME}/dbname.txt -f ${queriesdir}/q${t}b.sql > ${TPCDS_LOG_HOME}/${round}/q${t}b.log 2>&1
                        end=$(date +%s)
                        time=$(( $end - $start ))
                    ...
                    if [ -e "${queriesdir}/q${t}.sql" ]; then
                        start=$(date +%s)
                        $SPARK_PHIVE_HOME/bin/beeline -u jdbc:hive2://$(hostname):10000 -i ${TPCDS_LOG_HOME}/dbname.txt -f ${queriesdir}/q${t}.sql > ${TPCDS_LOG_HOME}/${round}/q${t}.log 2>&1
                        end=$(date +%s)
                        time=$(( $end - $start ))
        ...
        for round in $(seq $iteration);do
            ...
            for t in $(seq 22);do
                if [ -e "${queriesdir}/${t}.sql" ]; then
                    start=$(date +%s)
                    $SPARK_PHIVE_HOME/bin/beeline -u jdbc:hive2://$(hostname):10000 -i ${TPCH_LOG_HOME}/dbname.txt -f ${queriesdir}/${t}.sql > ${TPCH_LOG_HOME}/${round}/q${t}.log 2>&1
                    end=$(date +%s)
                    time=$(( $end - $start ))
    ...
    if [ "${workload}" = "tpcds" ]; then
        ...
        change_hibench_profile $hibenchWorkload
