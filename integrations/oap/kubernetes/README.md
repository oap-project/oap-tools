...
``` 
docker build --tag spark-centos:1.2.0 .
``` 
...
For example:
``` 
sh ./spark-pi.sh --master localhost:8443  --image oap-centos:1.2.0  --spark_conf ./conf
``` 
...
For exmaple:
``` 
sh ./spark-submit.sh --master localhost:8443  --image oap-centos:1.2.0  --spark_conf ./conf --name spark-pi --class org.apache.spark.examples.SparkPi  local:///opt/home/spark-3.1.1/examples/jars/spark-examples_2.12-3.1.1.jar 100
``` 
...
For example:
``` 
sh ./spark-thrift-server.sh start --image oap-centos:1.2.0  --spark_conf ./conf
``` 
